...
所有生成策略组默认每 240 秒通过 HTTPS 测速一次，并关闭按需测速（`lazy: false`），持续更新节点可用性与延迟。`tolerance: 100` 只会在候选节点比当前节点快超过 100 毫秒时切换，避免短时延迟波动导致频繁换节点；`fallback` 和 `load-balance` 不使用该字段。所有测速组保留 3 秒超时和连续 3 次失败阈值。代理提供者自身的订阅刷新配置不会被改写。
